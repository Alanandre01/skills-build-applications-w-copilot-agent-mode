...
const Teams = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return teams.filter(t => JSON.stringify(t).toLowerCase().includes(q));
  }, [teams, query]);
  ...
  if (loading) return <div className="text-center py-5">Chargement des équipes...</div>;

  return (
    <div className="container px-0">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h1 className="h3 m-0">Équipes</h1>
          ...
          <div className="table-responsive">
            <table className="table table-striped table-hover align-middle mb-0">
              ...
              <tbody>
                {filtered.map((t, idx) => (
                  <tr key={t.id || idx}>
                    <td>{getVal(t, ['id', 'pk'], idx + 1)}</td>
                    <td>{getVal(t, ['name', 'team_name', 'title'], '—')}</td>
                    <td>{getVal(t, ['members_count', 'members', 'size'], '—')}</td>
                    <td>{getVal(t, ['score', 'points'], '—')}</td>
                    <td>
                      <button className="btn btn-sm btn-outline-secondary me-2" onClick={() => setSelected(t)}>Voir</button>
                    </td>
                  </tr>
                ))}
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan="5" className="text-center text-muted py-4">Aucune équipe trouvée.</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <div className="modal-dialog modal-lg modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Détails de l'équipe</h5>
